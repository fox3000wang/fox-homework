window.onload = function () {
  ...
  let aP = getClass(oBut, 'b_tit');
  let oSmall = getClass(oTop, 'small')[0];
  ...
  for (i = 0; i < aLi.length; i++) {
    ...
    myAddEvent(aLi[i], 'mouseover', function () {
      let oDiv = this.getElementsByTagName('div')[0];
  ...
  function tab() {
    for (i = 0; i < oTli.length; i++) {
      (oTli[i].className = ''), startMove(oTli[i], { opacity: 40 });
    }
